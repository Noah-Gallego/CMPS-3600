...
int main(int argc, char *argv[], char *envp[])
{
    ...
    if (cpid == 0) {     /* CHILD */
        ...
        fflush(stdout); /* if you mix printf and write you must flush stdout */

        /* put the child in an infinite sleep loop */
        /* parent will wake up the child           */
        while (1)  {
            sleep(1);
        }
    ...
    if (cpid > 0) {   /* PARENT */
        ...
        status = 99;  
        /* make sure the child process has time to finish printf */
        sleep(nsecs);
        ...
        ret = kill(cpid, SIGSTOP);
        /* stop child process but do not terminate */
        if (ret < 0) {
            perror("kill SIGSTOP ");
        }
