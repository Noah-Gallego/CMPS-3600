...
void *traffic(void *arg)
{
	...
	while (1) {
		...
		if (overlap(&cars[carnum], &intersection)) {
			...
			sem_wait(&sems[semnum]); 
			readcount[semnum]++;

			if (readcount[semnum] == 1) {
				sem_wait(&sems[other_sem]);
			}

			sem_post(&sems[semnum]);
			...
			if (readcount[semnum] == 0) {
				sem_post(&sems[other_sem]);
			}
...
void render(void) {
	...
	y += 16;
	sprintf(str, " Car Passes 1: %i", g.passes[0]);
	XSetForeground(g.dpy, g.gc, 0x00ffff00);
	drawString(20, y, str);

	y += 16;
	sprintf(str, " Car Passes 2: %i", g.passes[1]);
	XSetForeground(g.dpy, g.gc, 0x00ffff00);
	drawString(20, y, str);

	y += 16;
	sprintf(str, " Car Passes 3: %i", g.passes[2]);
	XSetForeground(g.dpy, g.gc, 0x00ffff00);
	drawString(20, y, str);

	y += 16;
	sprintf(str, " Car Passes 4: %i", g.passes[3]);
	XSetForeground(g.dpy, g.gc, 0x00ffff00);
	drawString(20, y, str);
